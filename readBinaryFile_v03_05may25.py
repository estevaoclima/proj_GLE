...
#            spikeTime_16 = struct.unpack(f'>{fileSize_values_16}h', spkData)#just to check if was i16
        ...
        if tot_num_trls is None:
            raise ValueError("missing tot_num_trls! Must run .spikes BEFORE .swave!")
        ...
        with open(filename, 'rb') as f:
            ...
            while trls < tot_num_trls:
                ...
                #while True:# Read 38 values as 16-bit signed integers (2bytes per value *38 =76 bytes total wfm)
                    ...
                    # transform into array to store as a matrix of waveforms (them all have 38 values)
                    #waveforms.append(waveform)
                    waveform_arr = np.array(waveform)
                    waveforms.append(waveform_arr)
                    spikes_read += 1
                ...
                #result['waveforms'] = waveforms # wouldn't be better to save only a single matrix per trial 
                wfms_singleTrl = np.array(waveforms)
                result['waveforms'] = wfms_singleTrl
                
                trls += 1
            #    qwe[trls] = result# wouldn't be better to save only a single matrix per trial 
                qwe[trls] = wfms_singleTrl
        return qwe
